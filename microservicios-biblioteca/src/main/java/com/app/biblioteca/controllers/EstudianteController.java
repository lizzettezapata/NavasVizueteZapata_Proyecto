...
public class EstudianteController {
    ...
    public ResponseEntity<?> editar(@Valid @RequestBody Estudiante estudiante, @PathVariable Long id, BindingResult result) {
        ...
        Optional<Estudiante> estudianteDbOptional = service.findById(id);
        if (!estudianteDbOptional.isPresent()) {
            return ResponseEntity.notFound().build();
        }

        Estudiante estudianteDb = estudianteDbOptional.get();
        estudianteDb.setNombreEstudiante(estudiante.getNombreEstudiante());
        estudianteDb.setEmail(estudiante.getEmail());
        estudianteDb.setCarrera(estudiante.getCarrera());
        estudianteDb.setCelular(estudiante.getCelular());

        return ResponseEntity.status(HttpStatus.OK).body(service.save(estudianteDb));
    }
